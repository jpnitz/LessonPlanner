...
export function LessonPlannerOptions({
  ...
  const { settings, isLoading, isSaving, updateSettings } = useLessonPlanner();
    ...
    <section className="space-y-4 rounded-lg border border-border bg-surface p-3">
      ...
      {isLoading ? (
        ...
        <>
          ...
          {students.length > 0 ? (
            <div>
              <p className="mb-2 text-sm font-medium text-foreground">Students</p>
              ...
              <p className="mt-2 text-xs text-muted">
                Selected students filter the Curriculum page. Leave all unchecked
                to show every curriculum.
              </p>
            </div>
          ) : null}
          ...
          {!chatDraft.trim() ? (
            <p className="text-xs text-muted">
              Describe the subject you want in the chat box below first.
            </p>
          ) : null}
        </>
      )}
